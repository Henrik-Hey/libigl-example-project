	...
	Eigen::MatrixXi &covered_faces)
{
	...
	for (int f = 0; f < F_in.rows(); f++)
	{
		...
		bool isRegular =
			incident_faces[e1].size() == 2 && incident_faces[e2].size() == 2 && incident_faces[e3].size() == 2;
		...
		if (isRegular)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				if (F_in(nt1, i) != e1v1 && F_in(nt1, i) != e1v2)
					v1p = F_in(nt1, i);
				if (F_in(nt2, i) != e2v1 && F_in(nt2, i) != e2v2)
					v2p = F_in(nt2, i);
				if (F_in(nt3, i) != e3v1 && F_in(nt3, i) != e3v2)
					v3p = F_in(nt3, i);
			}
